...
from dl_cl_dataset import Lungdataset
from dl_cl_models import resnet18
...
def test(model, device, test_loader, criterion):
    ...
    with torch.no_grad():
        ...
        for data, target, cl_feature in test_loader:
            data = data.type(torch.FloatTensor)
            target = target.type(torch.FloatTensor)
            cl_feature = cl_feature.type(torch.FloatTensor)
            data, target, cl_feature = data.to(device), target.to(device), cl_feature.to(device)
            output = model(data, cl_feature)
...
def train(log_interval, model, device, train_loader, criterion, optimizer, epoch):
    model.train()
    for batch_idx, (data, target, cl_feature) in enumerate(train_loader):
        data = data.type(torch.FloatTensor)
        target = target.type(torch.FloatTensor)
        cl_feature = cl_feature.type(torch.FloatTensor)
        # 将训练数据加载至硬件
...
if __name__ == '__main__':
    ...
    for fold_n in range(n_fold):
        save_model_str = 'res18_dlcl_bs{}_lb{}_wabs_fold{}'.format(batch_size, label_dict[label_idx], fold_n)
        ...
        if load_pretrained:
            check_point = torch.load('weights/res18_bs{}_lb{}_wabs_fold{}.pth'.format(batch_size,
                                                                                  label_dict[label_idx],
                                                                                  fold_n))
            model_para = model.state_dict()
            for key in model_para:
                if key in check_point:
                    model_para[key] = check_point[key]
